...
#Create column vector of option prices C_hat
        ...
        #Must incorporate lowest and highest option prices, which are not in matrix
            C_min_call = 0.5*((sigma**2)*((1)**2)*deltaT - (r - divYield)*(1)*deltaT)*C_0 + A[0,0]*C_hat_call[0] + A[0,1]*C_hat_call[1]
            C_max_call = A[J-2,J-3]*C_hat_call[J-3] + A[J-2,J-2]*C_hat_call[J-2] + 0.5*((sigma**2)*((J-1)**2)*deltaT + (r - divYield)*(J-1)*deltaT)*(S_max - K*math.exp(-r*m*deltaT))
